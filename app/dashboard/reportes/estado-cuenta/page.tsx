  ...
  return (
    <div>
      ...
      <div className="breadcrumb">
        ...
        <span className="breadcrumb-current">Estado de Cuenta</span>
      </div>
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Busca un cliente y selecciona un prestamo para generar el estado de cuenta</p>
        </div>
      </div>
      ...
      <div className="card">
              ...
              {searchLoading ? (
                ...
                clients.map((client) => (
                  <div key={client.id} className="dropdown-item" onClick={() => handleSelectClient(client)}>
                    ...
                    <div>
                      <span className="dropdown-name">{client.firstName} {client.lastName ?? ""}</span>
        ...
        {selectedClient && (
          <div className="selected-client-info">
            ...
            <div>
              <span className="selected-name">{selectedClient.firstName} {selectedClient.lastName ?? ""}</span>
      ...
      {selectedClient && (
        <div className="card">
          <div className="step-header">
            ...
            <h2 className="step-title">Seleccionar Prestamo</h2>
          </div>
          {loansLoading ? (
            <div className="loading-text">Cargando prestamos...</div>
          ) : loans.length === 0 ? (
            <div className="empty-text">Este cliente no tiene prestamos registrados</div>
          ) : (
                ...
                return (
                  <div key={loan.id} className={`loan-item ${isSelected ? "loan-item--selected" : ""}`} onClick={() => setSelectedLoan(loan)}>
                    <div className="loan-item-left">
                      ...
                      <div>
                        <span className="loan-item-amount">RD$ {fmt(Number(loan.principalAmount))}</span>
                        <span className="loan-item-date">{fmtDate(loan.createdAt)}</span>
                      </div>
                    </div>
                    <div className="loan-item-right">
                      <span className="loan-item-status" style={{ color: STATUS_COLORS[loan.status] || "#6b7280" }}>
      ...
      {selectedLoan && (
        <div className="card">
          <div className="step-header">
            ...
            <h2 className="step-title">Generar Estado de Cuenta</h2>
          </div>
          <div className="summary-grid">
            <div className="summary-item">
              <span className="summary-label">Cliente</span>
              <span className="summary-value">{selectedClient!.firstName} {selectedClient!.lastName ?? ""}</span>
            </div>
            <div className="summary-item">
            ...
            <div className="summary-item">
              <span className="summary-label">Monto del prestamo</span>
              <span className="summary-value">RD$ {fmt(Number(selectedLoan.principalAmount))}</span>
            </div>
            <div className="summary-item">
              <span className="summary-label">Capital pendiente</span>
              <span className="summary-value">RD$ {fmt(Number(selectedLoan.remainingCapital))}</span>
            </div>
            <div className="summary-item">
              <span className="summary-label">Frecuencia</span>
              <span className="summary-value">{FREQ_LABELS[selectedLoan.paymentFrequency] ?? selectedLoan.paymentFrequency}</span>
            </div>
            <div className="summary-item">
              <span className="summary-label">Estado</span>
              <span className="summary-value" style={{ color: STATUS_COLORS[selectedLoan.status] || "#6b7280" }}>
                {STATUS_LABELS[selectedLoan.status] ?? selectedLoan.status}
              </span>
            </div>
          </div>
          <div className="action-buttons">
            <ReportButton type="estado-cuenta" entityId={selectedLoan.id} label="Ver Estado de Cuenta" variant="primary" size="lg" mode="preview" />
            <ReportButton type="estado-cuenta" entityId={selectedLoan.id} label="Descargar PDF" variant="outline" size="lg" mode="download" />
          </div>
        </div>
      )}
